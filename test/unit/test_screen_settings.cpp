...
TEST_CASE("focus skips headers and inert rows in both directions") {
  ...
  scr.onEvent(kUp);
  CHECK(focusedLabel(scr) == "Sleep after");
  // And UP from the first focusable row does not climb into the DEVICE header or
  // the typography rows above it.
  scr.onEvent(kUp);
  CHECK(focusedLabel(scr) == "Sleep after");
}

TEST_CASE("a move that changes nothing returns none, so no refresh is spent") {
  // On this panel a repaint is at least 520 ms, and spending it to draw an
  // identical screen is what makes the end of a list feel like a stuck button.
  SettingsScreen scr = sized(Settings{}, nullptr);
  const reader::Action a = scr.onEvent(kUp);  // already at the first focusable row
  CHECK(a.kind == reader::Action::Kind::None);
  const reader::Action b = scr.onEvent(kDown);
  CHECK(b.kind == reader::Action::Kind::Redraw);
}
...
TEST_CASE("setFocus refuses a header or an inert row") {
  ...
  // press, so the restore is refused and the shell reports it -- the same contract
  // ScrollWindow::setFocus uses.
  SettingsScreen scr = sized(Settings{}, nullptr);
  ...
  CHECK(scr.setFocus(7));         // Sleep after
  CHECK(focusedLabel(scr) == "Sleep after");
}
