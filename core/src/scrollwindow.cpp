...
// on.
bool ScrollWindow::moveFocus(int delta, bool held) {
  if (focus_.count() == 0 || visible_ == 0) return false;
  const int wasFirst = first_;
  // Straight through: Focus owns clamp-versus-wrap and a window only follows.
  const bool moved = focus_.move(delta, held);
  clampWindow();
