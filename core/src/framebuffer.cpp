...
void Framebuffer::fillRect(int x, int y, int w, int h, bool white) {
  // BYTE-WISE, AND IT WAS THE BIGGER OF THE TWO. The veil was the recorded
  // suspicion for what made an overlay expensive; measured on the device this was:
  // fillRect moved ~200,000 pixels in 158 ms -- 790 ns a pixel -- which is 62% of
  // an item-actions render and 53% of the reader menu's, against veilRect's 24 ns
  // a pixel for the WHOLE frame. Same defect, same rewrite, 33x apart: a bounds
  // check, a byteIndex (a division under rotation), a bitMask (a modulo) and a
  // read-modify-write, per pixel.
  //
  // The span stays: it is what caught this, and it is what will catch the next one.
