...
bool SettingsScreen::setFocus(int index) {
  ...
  // ScrollWindow::setFocus's bool means "something CHANGED", and this function's
  // means "the restore LANDED". They are not the same and returning the first for
  // the second is a bug the shell already carries a comment about: restoring focus
  // onto the row a screen is already on changes nothing and is a perfectly
  // successful restore. The first version of this propagated it and reported
  // failure for the commonest case there is -- the default focus.
  window_.setFocus(index);
  syncVm();
  return window_.focus() == index;
}
...
Action SettingsScreen::moveFocus(int dir) {
  ...
  for (;;) {
    ...
    if (next < 0 || next >= static_cast<int>(kItems.size())) return Action::none();
    i = next;
