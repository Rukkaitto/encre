/* --- Vendored, unmodified below this block ----------------------------------
/
/ TJpgDec R0.03, by ChaN. Its licence is at the top of the block below and is the
/ upstream author's own, kept verbatim -- "no restriction on use", with the one
/ condition that a redistribution of the source retains that notice.
/
/ From https://elm-chan.org/fsw/tjpgd/arc/tjpgd3.zip, whose sha256 is
/ 052fe3efbc9a8be29f31597ad009c5b51a4f6905878eb28569e0ab3d46d0c013; src/tjpgd.c,
/ src/tjpgd.h and src/tjpgdcnf.h of that archive are the three files taken.
/
/ NOT ONE BYTE BELOW THIS BLOCK IS CHANGED -- checked rather than asserted:
/   tail -n +32 third_party/tjpgd.c | shasum -a 256
/ is 3a0091b6e1e6407f18633c17c19b5791b878d3ede4282005222bc41050e347a9, the
/ archive's own src/tjpgd.c. CRLF line endings are the upstream file's and are
/ kept for that reason. So a version bump is a straight
/ replace. Everything this project configures is a macro tjpgdcnf.h provides, and
/ that file -- which IS edited, and says which of its values are ours -- is the
/ only one of the three to read before touching this one.
/
/ VENDORED RATHER THAN WRITTEN, which is the opposite of the call this project
/ made for DEFLATE. The reasons there were stb's one-shot API and its 6,608-byte
/ single stack frame (see core/include/reader/inflate_stream.h); neither applies
/ here, because TJpgDec already has the memory model this pipeline needs -- a
/ ~3 KB pool the caller owns, an output callback per MCU, and free 1/2, 1/4 and
/ 1/8 scaling out of the IDCT. And JPEG's edge cases are numerous in a way
/ DEFLATE's are not: a wrong upsample is a subtly wrong PICTURE rather than a
/ crash, which is the hardest class of defect for this project's tests to catch.
/
/ Wrapped by core/src/jpegd.cpp, which is the only file that includes it.
/--------------------------------------------------------------------------- */
