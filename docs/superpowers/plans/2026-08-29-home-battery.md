...
| `core/src/icons_data.h` | Regenerated by `make icons`. Note this is under `core/src/`, NOT `core/include/reader/` — the Makefile's `icons` target names it. |
| `core/include/reader/app.h`, `core/src/app.cpp` | `App::markDirty()`. |
...
## Task 7: `App::markDirty()`

**Named `markDirty`, not `requestRepaint`.** `App` already has four
`xRequested()`/`clearXRequest()` pairs -- `sleepRequested`, `retryRequested`,
`openRequested` -- and every one is a PULL LATCH: something sets it, the shell
notices, acts, and clears it. This is not that. It sets the same `dirty_` the
shell already polls every loop, so it is the third member of the
`dirty()`/`clearDirty()` pair, and it is named to read like one.

**It also invalidates the partial-repaint record, and that is load-bearing, not
belt-and-braces.** Every previous route to `dirty_` was a dispatch, which only
changes the top screen's own state, or a push/pop, which also sets
`transition_` -- an invariant `canRenderTopOnly` quietly rests on: every clause
was written assuming `dirty_` only ever turns true for a reason the TOP screen
knows about. This is the first route that dirties the app for a reason
unrelated to whatever is on top, and with an overlay up every OTHER clause
still passes -- the frame, the top, the depth and the footprint are all
unchanged. Left as a bare `dirty_ = true`, `renderTopOnly()` would repaint only
the overlay's own pixels and leave whatever the call was actually for stale on
glass, with `dirty_` cleared and nothing left to correct it. `ItemActions` and
`DeleteConfirm` both carry non-zero footprints, so this is reachable on real
screens, not theoretical -- it cannot fire through Task 8's call sites, which
are gated on Home being on glass and Home is never an overlay, but the API
itself must not depend on every future caller remembering that. `-fno-exceptions`
firmware rules out an `assert` as the fix (it can compile out); the fix is
structural: reset the private `painted_` record so the next paint goes through
`App::render`, full stack, whatever is on top.

**Files:**
- Modify: `core/include/reader/app.h`, `core/src/app.cpp`
- Modify: `test/unit/test_app.cpp` -- NOT `test_screens.cpp`, whose cases are
  about navigation semantics. Every `dirty()`/`transition()`/`canRenderTopOnly()`
  test already lives in `test_app.cpp`, including one nearly identical in shape
  ("a redraw is dirty but is not a transition"), and `PartialFixture` -- the one
  state a partial repaint is legal in -- is already there too.

- [ ] **Step 1: Write the failing tests**

Beside `TEST_CASE("a redraw is dirty but is not a transition")` in
...
And beside the other `PartialFixture` tests -- e.g. after
...
TEST_CASE("markDirty invalidates the partial-repaint record, even over an overlay") {
  ...
  // Redraw. Every clause canRenderTopOnly checks was written assuming dirty_
  // only ever turns true for a reason the TOP screen knows about -- so with an
  // overlay up, markDirty's dirty_ = true alone would satisfy every one of
  // them: the frame, the top, the depth and the footprint are all still what
  // they were. renderTopOnly would then repaint the overlay's own pixels and
  // leave whatever markDirty was actually called for stale on glass, with
  // dirty_ cleared and nothing left to correct it. ItemActions and
  // DeleteConfirm both carry non-zero footprints, so this is reachable on real
  // screens, not theoretical.
  PartialFixture fx;
...
Expected: `'class reader::App' has no member named 'markDirty'`.
...
- [ ] **Step 5: Commit, then prove both tests bite**

Commit, then run two mutations in turn, confirming each fails and restoring
(`git checkout core/src/app.cpp && touch core/src/app.cpp`) between them:

1. Also set `transition_ = true;` in `markDirty` -- fails
   `CHECK_FALSE(app.transition())` in the first test.
2. Remove the `painted_ = PaintRecord{};` line, leaving only `dirty_ = true;` --
   fails `CHECK_FALSE(fx.app.canRenderTopOnly(...))` in the second test.
...
git diff --cached --stat
git commit -m "$(cat <<'EOF'
feat(core): App::markDirty, and it cannot leave a partial repaint

App already carries four xRequested/clearXRequest pairs, and every one is
a pull latch: something sets it, the shell notices, acts, clears it. This
is not that -- it sets the same dirty_ the loop already polls, so it is
the third member of the dirty/clearDirty pair.

Every previous route to dirty_ was a dispatch, which only changes the top
screen's own state, or a push/pop, which also sets transition_ -- an
invariant canRenderTopOnly quietly rests on. This is the first that
dirties for a reason unrelated to the top screen, and with an overlay on
top every other clause still passed: the overlay alone would repaint,
whatever the caller wanted redrawn would stay stale, and dirty_ would be
cleared with nothing left to correct it. ItemActions and DeleteConfirm
both carry non-zero footprints, so it was reachable.

Gated call sites would have avoided it; an API that cannot be misused is
better than a comment saying not to.
