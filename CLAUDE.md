...
|--:|---|---|
| 98,304 | `BlockReader::next` → `Block::text` | **nobody** — see below |
...
- **`Block::text`, which is the largest unguarded allocation on this path at 98,304
  bytes** against `kMaxBlockBytes` of 64 KB — a cap this tree still carries and which
  a 42,152-byte reading floor cannot honour. It belongs to
  `claude/block-cap-heap-bound`, which reserves it through a nothrow probe and derives
  the cap from the heap; guarding it here would have been the same file twice in two
  branches.
